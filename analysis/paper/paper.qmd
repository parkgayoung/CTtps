...
Our results show positive correlation between attributes. Panel A and B of @fig-corr-analysis indicate that some attributes such as BL and ML have stronger correlation than others. The correlations between attributes became stronger over time as we can clearly see in the Panel C of @fig-corr-analysis. The median for Phase 2 is much lower than `r threshold_value_cor`, which @bettinger1999point interpreted as a threshold to evaluate transmission biases. Therefore, we assume that the earlier transmission for stemmed points was more likely the results of guided variation. In Phase 3, the median is close to `r threshold_value_cor`, which means the Phase 3 could be more associated with indirect bias. 

```{r}
#| label: fig-corr-analysis
#| fig-cap: "Correlation analysis by chronological phases in the Korean Late Paleolithic period. A: Correlation coefficient between attributes for Phase 1. B. Correlation coefficient between attributes for Phase 2. C. Correlation coefficient for the second and third chronological phases. The grey points represent correlation between two attributes. "
#| fig-width: 7
#| fig-height: 3
