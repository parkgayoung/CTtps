...
@fig-plot-cv-over-time shows that the directional trend in CV values of artefact attributes is complicated. Half of the attributes are below our threshold value of 25, and half are above. Only body length changes from <25 to >25 over time, indicating more guided variation. For those attributes where the CV is <25, the trend is decreasing CV values from Phase 2 to Phase 3 for maximum width and tang length, indicating increasing influence of indirect bias for these attributes. For stem length, stem width and tang length, CV values remain above 25, and show a slight trend to increase over times, suggesting increased influence of guided variation for these attributes. 
