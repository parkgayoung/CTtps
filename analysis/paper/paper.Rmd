...
# phase 2
fviz_pca_var(xx_pca2, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"))
# phase 3
fviz_pca_var(xx_pca3, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"))
```
...
SYG6_xx_pca1 <- 
  data.frame(SYG6_xx_pca$x,
             stage = as.factor( SYG6_df_sitename$SPstage.Stage[SYG6_df_sitename$SPstage.Stage !=1]))
...
fviz_pca_var(SYG6_xx_pca, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"))
...
SYG_1 site data and analysis

```{r}

# artifacts from SYG_1 site
library(stringr)
SYG_1df <- 
  df %>% 
  filter(str_detect(files, "SYG_1"))


SYG_1df_sitename <- SYG_1df %>% 
  mutate (files = str_remove_all(files, "A_|1|2|.tps")) %>% 
  separate (files, into = c("sitename", "files"), sep = "_") %>% 
  select (-files)
...
#simpler way to calculate Correlataion, you can use
SYG_1cor <- cor(SYG_1DF)

#a scatterplot matrix of Correlation
pairs(SYG_1DF)


cplot_SYG_1 <- corrplot(SYG_1cor, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)
...
SYG_1PCA <- prcomp(SYG_1DF, center = TRUE, scale = TRUE)
...
SYG_1xx_pca1 <- 
  data.frame(SYG_1xx_pca$x,
             stage = as.factor( SYG_1df_sitename$SPstage.Stage[SYG_1df_sitename$SPstage.Stage !=1]))
...
fviz_pca_var(SYG_1xx_pca, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"))


SYG_1PC1_v <- fviz_contrib(SYG_1xx_pca, choice = "var", axes = 1, top = 7)
SYG_1PC2_v <- fviz_contrib(SYG_1xx_pca, choice = "var", axes = 2, top = 7)

grid.arrange(SYG_1PC1_v, SYG_1PC2_v, nrow =1)
...
SYG_4 site data and analysis

```{r}

# artifacts from SYG_4 site
library(stringr)
SYG_4df <- 
  df %>% 
  filter(str_detect(files, "SYG_4"))


SYG_4df_sitename <- SYG_4df %>% 
  mutate (files = str_remove_all(files, "A_|1|2|.tps")) %>% 
  separate (files, into = c("sitename", "files"), sep = "_") %>% 
  select (-files)
...
#simpler way to calculate Correlataion, you can use
SYG_4cor <- cor(SYG_4DF)

#a scatterplot matrix of Correlation
pairs(SYG_4DF)


cplot_SYG_4 <- corrplot(SYG_4cor, type = "upper", order = "hclust", 
         tl.col = "black", tl.srt = 45)
...
SYG_4PCA <- prcomp(SYG_4DF, center = TRUE, scale = TRUE)
...
SYG_4xx_pca1 <- 
  data.frame(SYG_4xx_pca$x,
             stage = as.factor( SYG_4df_sitename$SPstage.Stage[SYG_4df_sitename$SPstage.Stage !=1]))
...
fviz_pca_var(SYG_4xx_pca, col.var = "contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"))

SYG_4PC1_v <- fviz_contrib(SYG_4xx_pca, choice = "var", axes = 1, top = 7)
SYG_4PC2_v <- fviz_contrib(SYG_4xx_pca, choice = "var", axes = 2, top = 7)

grid.arrange(SYG_4PC1_v, SYG_4PC2_v, nrow =1)
```
