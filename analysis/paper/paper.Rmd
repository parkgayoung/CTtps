...
- Two models: Guided variation and Model-based bias =>  Multi-point Origin (in situ) and Single-point Origin (migration model)
...
# I can move the library(pkg) to the top with other libraries but I'm leaving here for my reference for a while
library(Hmisc) 
...
library(ggfortify) 
ap <- 
autoplot(prcomp(DF, center = TRUE, scale = TRUE), 
         data = df_sitename, 
         colour = as.factor(df_sitename$SPstage.Stage))


pca_col <- data.frame(PCA$x, phase = as.factor(df_sitename$SPstage.Stage))
...
xx_pca1 <- data.frame(xx_pca$x,stage = as.factor( df_sitename$SPstage.Stage[df_sitename$SPstage.Stage !=1]))
...
PCA with corrplot package to understand the individual variables
```{r}
